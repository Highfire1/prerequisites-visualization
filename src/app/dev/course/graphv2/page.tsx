	...
	return (
		<div
			className={`shadow-lg rounded-lg border ${
				n.highlight ? "border-blue-500 ring-2 ring-blue-200" : "border-gray-300"
			} bg-white text-gray-900 select-none`}
			style={{
				width: n.width,
...
export default function GraphFunctionalPage() {
			...
			if (settings.useParentAnchors) {
				...
				const childInfos: ChildInfo[] = arr.map((id) => {
					...
					const candidates = nodesRef.current
						.filter((p) => (depthRef.current.get(p.id) ?? 0) === d - 1 && listDirectPrereqCourses(p.id).includes(id))
						.map((p) => p.id)
	...
	const upsertNode = useCallback(
		...
		[settings.ringBase, settings.ringStep]
	);
	...
	const handleButtonClick = useCallback(
		(parentId: string, courseId: string) => {
			...
			[ensureLink, removeEphemeralLink, upsertNode, settings.pinClicked]
	);
	...
	const handleButtonHoverOut = useCallback(
		...
		}, [nodes.length, links.length, settings]);
		...
		<div className="w-full h-[100vh]">
			...
			>
					...
					<div data-pan-block className="absolute bottom-3 right-3 z-20 flex flex-col items-stretch gap-2">
						<div className="bg-white/90 backdrop-blur rounded-lg border border-gray-200 shadow overflow-hidden">
							<button
								className="block w-10 h-10 leading-10 text-center border-b border-gray-200 hover:bg-gray-50 select-none"
								onClick={() => {
									const el = containerRef.current;
									if (!el) return;
									const rect = el.getBoundingClientRect();
							...
							<button
								className="block w-10 h-10 leading-10 text-center hover:bg-gray-50 select-none"
								onClick={() => {
									const el = containerRef.current;
									if (!el) return;
									const rect = el.getBoundingClientRect();
						...
						<button
							className="bg-white/90 backdrop-blur rounded-full border border-gray-200 shadow w-10 h-10 hover:bg-gray-50 select-none"
							onClick={() => setView({ x: 0, y: 0, k: 1 })}
							title="Reset view"
						>
							🏠
